...
export class Channel {
	...
	public async watch(): Promise<boolean> {
		...
		if (this.nextWatch >= Date.now()) return false
		if (Campaign.trackMinutesWatched === 0) await this.listen()
	...
	public async claimPoints(claimID: string = this.pointInstanceID!): Promise<boolean> {
		if (!container.client.config.isClaimPoints || !claimID) return false

		const res = await container.api.claimPoints({ channelID: this.id, claimID })
		if (!strictHas(res, 'data.claimCommunityPoints')) return false
	...
	public async claimMoments(momentID: string = this.momentInstanceID!): Promise<boolean> {
		if (!container.client.config.isClaimMoments || !momentID) return false
		...
		try {
			...
			if (isErrorLike<RequestError>(error) && isObjectLike(error.response)) {
				if (error.response.statusCode === 404) {
					...
					if (!strictHas(channel, 'data.user.stream.id')) return false
	...
	private readonly topics = [WsEvents.ChannelMoment, WsEvents.ChannelStream, WsEvents.ChannelUpdate]
}
