...
export const SocketWorkflow = (state: MainState, configStore: StoreClient<ClientConfig>) =>
  Effect.gen(function* () {
    ...
    const processMessage = (msg: SocketMessage) =>
      Effect.gen(function* () {
        ...
        if (Option.isNone(currentCampaign) || Option.isNone(currentChannel)) {
          return;
        }
        ...
        yield* api.writeDebugFile(msg, `${msg.topicType}-${msg.type || Date.now()}`);

        const logEmit = (topic: string) => Effect.logDebug(chalk`{white ${JSON.stringify(msg)}} | AppSocket: Emitted ${topic}.${msg.topicId}`);
