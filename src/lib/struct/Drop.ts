...
export class Drop {
	...
	public constructor(drop: DropContext) {
		this.id = drop.id
		this.name = drop.benefitEdges[0].benefit.name.trim()
	...
	public get isStatus(): DropStatus {
		if (this.isClaimed) return { expired: true, upcoming: false }
		else if (this.hasAward) return { expired: false, upcoming: false }
	...
	public async claimDrops(dropID: string = this.dropInstanceID!): Promise<boolean> {
		if (!container.client.config.isClaimDrops || !dropID) return false

		const res = await container.api.claimDrops(dropID)
		if (!strictHas(res, 'data.claimDropRewards')) return false
